...
| 版本 | 说明 |
|------|------|
| **v1.3.1** | 截图稳定性优化、全新紧凑界面、多滚动区域切换动效与对齐修复 |
...
2. 状态显示页面尺寸 → 即可截图
3. 多滚动区域页面：在“滚动区域”列表中用右侧开关选择要截的容器，用左侧圆点选择决定最终高度的主容器
4. 选择格式 → 调整延迟 → 点击 **"截取长图"**
5. 文件保存到 `~/Downloads/SnapLong/`

## v1.3.1 更新

- 截图流程更稳健：限制截图节奏、增加捕获重试，并在成功、失败或中断后恢复页面与滚动容器的位置。
- 多滚动区域选择器固定右侧操作列；切换主容器时，开关不会再横向跳动。
- 弹窗改为紧凑的工具型界面，重新设计扩展图标，并补充键盘焦点、状态提示和减少动态效果偏好支持。
